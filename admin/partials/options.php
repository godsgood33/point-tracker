...
<div>
    <form method='post' action='#'>
        <input type='hidden' name='_wpnonce'
            value='<?php print wp_create_nonce('pt-update-options'); ?>' />
        <div class='notice notice-warning'><?php
            print __('Not requiring a login opens challenge participants to potential unauthorized activity deletions', 'point-tracker');
        ?>
        </div>
        <label for='require-login'>Login Required? </label>
        <input type='checkbox' name='require-login' id='require-login' value='1' <?php print(get_option('pt-require-login', 0) ? "checked" : ''); ?>
        /><br />
