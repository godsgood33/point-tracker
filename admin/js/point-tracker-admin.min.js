...
}function beforeAjaxSend(){$("#loading,#waiting").show();$("#waiting").animate({"opacity":"0.5"},300,"linear");}function ajaxComplete(){$("#loading,#waiting").hide();$("#waiting").animate({"opacity":"0"},300,"linear");}var opts={lines:25,length:25,width:5,radius:50,scale:1,corners:1,color:"#000",opacity:0.25,rotate:0,direction:1,speed:0.5,trail:60,fps:20,zIndex:2000000000,className:"spin-thingy",top:"50%",left:"50%",shadow:false,hwaccel:false,position:"absolute"};var target,spinner,table1,table2;$("#start-date,#end-date,#act-start,#act-end").datepicker({dateFormat:my_object.date_format});
if($("#loading").length){target=document.getElementById("loading");spinner=new Spinner(opts).spin(target);}dialog=$("#dialog-form").dialog({autoOpen:false,height:400,width:350,modal:true,buttons:{"Add Leader":addLeader,Cancel:function(){dialog.dialog("close");}},close:function(){form[0].reset();}});form=dialog.find("form").on("submit",function(event){event.preventDefault();addLeader();});$("#add-leader").button().on("click",function(){dialog.dialog("open");});$(".tooltip-field").tooltip({show:{effect:"slideDown",delay:100},hide:{effect:"slideUp",delay:250}});
});