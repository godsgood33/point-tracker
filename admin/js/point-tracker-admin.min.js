jQuery(function($){function get_challenge(){if(!$("#challenge").val()){reset_challenge_form();return}$.ajax(ajaxurl,{data:{action:"get-challenge","chal-id":$("#challenge").val()},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return}$("#name").val(data.name);$("#start-date").val(data.start);$("#end-date").val(data.end);$("#desc").text(data.desc);$("#act-count").text(data.act_count);$("#part-count").text(data.part_count);$("#approval").prop("checked",stringToBoolean(data.approval));
if(data.winner){$("#winner").html("Winner is "+data.winner);$("#winner").show()}else{$("#winner").html("");$("#winner").hide()}if($("#approval").is(":checked")){$("#link").html('<a href="/index.php/challenge-list/?chal='+data.short_link+'" target="_blank">'+data.short_link+"</a>")}else{$("#link").html('<a href="/index.php/challenge/?chal='+data.short_link+'" target="_blank">'+data.short_link+"</a>")}},error:function(xhr,status,error){console.error(error)},dataType:"json",method:"post"})}function save_challenge(){$.ajax(ajaxurl,{data:{action:"save-challenge","chal-id":$("#challenge").val(),name:$("#name").val(),"start-date":$("#start-date").val(),"end-date":$("#end-date").val(),desc:$("#desc").val(),approval:($("#approval").is(":checked")?"1":"0")},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return
}if(!$("#challenge").val()){$("#challenge").append("<option value='"+data.id+"' selected>"+data.name+"</option>")}$("#link").html('<a href="/index.php/challenge-list/?chal='+data.uid+'" target="_blank">'+data.uid+"</a>")},error:function(xhr,status,error){console.error(error)},dataType:"json",method:"post"})}function delete_challenge(){if(!$("#challenge").val().length){return}if(!confirm("Are you sure you want to delete this challenge?")){return}$.ajax(ajaxurl,{data:{action:"delete-challenge","chal-id":$("#challenge").val(),security:$("#_wpnonce").val()},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return
}$("#challenge option:selected").remove();reset_challenge_form()},error:function(xhr,status,error){console.error(error)},dataType:"json",method:"post"})}function remove_winner(){if(!$("#challenge").val()){return}$.ajax(ajaxurl,{data:{action:"remove-winner","chal-id":$("#challenge").val()},success:function(data){if(!check_for_error(data)){return}$("#winner").html("");$("#winner").hide()},error:function(xhr,status,error){console.log(error)},dataType:"json",method:"post"})}function reset_challenge_form(){$("#challenge,#name,#start-date,#end-date").val("");
$("#approval").prop("checked",false);$("#act-count,#part-count").html(0);$("#link,#winner").html("");$("#desc").text("");$("#winner").hide()}$("#challenge").change(get_challenge);$("#save-challenge").click(save_challenge);$("#delete-challenge").click(delete_challenge);$("#remove-winner").click(remove_winner);function get_challenge_activities(){if(!$("#challenge_activities").val().length){return}$.ajax(ajaxurl,{data:{action:"get-activities","chal-id":$("#challenge_activities").val()},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return
}if($.fn.DataTable.isDataTable("#activity-table")){table1.destroy();$("#activity-table").empty()}table1=$("#activity-table").DataTable({data:data.data.slice(0),columns:data.columns.slice(0),paging:false,buttons:["copyHtml5","csv","excel","pdf","print"],dom:"Bfrtip"});$(".tooltip-field").tooltip({show:{effect:"slideDown",delay:100},hide:{effect:"slideUp",delay:250}});if(data.group_msg){$("#group-msg").show()}else{$("#group-msg").hide()}reset_activity_form()},error:function(xhr,status,error){console.error(error)
...
}$.ajax(ajaxurl,{data:{action:"get-participants","chal-id":$("#challenge_participants").val()},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return}if($.fn.DataTable.isDataTable("#participant-table")){table1.destroy();$("#participant-table").empty()}table1=$("#participant-table").DataTable({data:data.data.slice(0),columns:data.columns.slice(0),order:[[4,"desc"]],buttons:["copyHtml5","csv","excel","pdf","print"],dom:"Bfrtip"});$(".approve").click(approve_participant);
$(".fa-trash-alt").click(remove_participant);$(".fa-trophy").click(mark_winner);if($.isNumeric(data.winner)){$(table1.row(data.winner).nodes()[0]).addClass("winner")}},error:function(xhr,status,error){console.error(error)},dataType:"json",method:"post"})}function approve_participant(){var uid=$(this).data("user-id");$.ajax(ajaxurl,{data:{action:"approve-participant","chal-id":$("#challenge_participants").val(),"user-id":uid},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return
}$(this).prop("checked",true)},error:function(xhr,status,error){console.error(error)},dataType:"json",method:"post"})}function remove_participant(){if(!$("#challenge_participants").val()){return}var uid=$(this).data("user-id");$.ajax(ajaxurl,{data:{action:"remove-participant","chal-id":$("#challenge_participants").val(),"user-id":uid,security:$("#_wpnonce").val()},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return}table1.row().remove().draw()},error:function(xhr,status,error){console.error(error)
},dataType:"json",method:"post"})}function clear_participants(){$.ajax(ajaxurl,{data:{action:"clear-participants","chal-id":$("#challenge_participants").val(),security:$("#_wpnonce").val()},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return}if($.fn.DataTable.isDataTable("#participant-table")){table1.destroy();$("#participant-table").empty()}},error:function(xhr,status,error){console.error(error)},dataType:"json",method:"post"})}function add_participant(){if(!$("#challenge_participants").val()){return
}var numeric=/^\d+$/;var ret=true;$("#msg div").remove();if(!$("#member-id").val()||!$("#member-id").val().match(numeric)){$("#msg").append("<div>Member ID must be numeric</div>");ret=false}if(!$("#user-name").val()){$("#msg").append("<div>Must add users name</div>");ret=false}if(!$("#user-email").val()){$("#msg").append("<div>Must add the users email</div>");ret=false}if(!ret){$("#msg").show(300);$("#msg").addClass("err-msg");setTimeout(function(){$("#msg").hide(300)},3000);return ret}$.ajax(ajaxurl,{data:{action:"add-participant","chal-id":$("#challenge_participants").val(),"member-id":$("#member-id").val(),"user-name":$("#user-name").val(),"user-email":$("#user-email").val()},beforeSend:beforeAjaxSend,complete:ajaxComplete,success:function(data){if(!check_for_error(data)){return
}table1.row.add({approved:"<input type='checkbox' class='approved' checked />",memberid:$("#member-id").val(),name:data.name,email:data.email,totalPoints:0,action:"<i class='far fa-trash-alt' title='Remove this participant from the activity' data-user-id='"+data.user_id+"'></i>&nbsp;&nbsp<i class='fas fa-trophy' title='Mark participant as challenge winner' data-user-id='"+data.user_id+"'></i>"}).draw(false);$(".fa-trash-alt").off("click");$(".fa-trash-alt").on("click",remove_participant);$(".fa-trophy").off("click");
$(".fa-trophy").on("click",mark_winner);$("#admin-add-participant").hide(300)},error:function(xhr,status,error){console.error(data.error)},dataType:"json",method:"post"})}function mark_winner(){if(!$("#challenge_participants").val()){return}var user_id=$(this).data("user-id");button=$(this);$.ajax(ajaxurl,{data:{action:"mark-winner","chal-id":$("#challenge_participants").val(),"user-id":user_id},success:function(data){if(!check_for_error(data)){return}$(button).closest("tr").addClass("winner")},error:function(xhr,status,error){console.error(error)
