...
class PointTrackerPublic
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        add_shortcode('challenge', [
            $this,
            'displayChallengePage'
        ]);
        add_shortcode('challenge_list', [
            $this,
            'displayChallengeListPage'
        ]);
    ...
    public function enqueueStyles()
    {
        ...
        wp_enqueue_style(
            $this->pluginName,
            plugin_dir_url(__FILE__) . "css/point-tracker-public.min.css",
            [],
            $this->version,
            'all'
        );
        ...
        wp_enqueue_style(
            'dt-jqueryui-css',
            plugin_dir_url(__DIR__) . "includes/datatables/DataTables-1.10.9/css/dataTables.jqueryui.min.css"
        );
        wp_enqueue_style(
            'dt-buttons-css',
    ...
    public function enqueueScripts()
    {
        ...
        wp_enqueue_script($this->pluginName, plugin_dir_url(__FILE__) . "js/point-tracker-public.min.js", [
            'jquery'
        ], $this->version, false);
        wp_localize_script($this->pluginName, 'ajax_object', [
            'ajax_url' => admin_url('admin-ajax.php'),
            'chal_page' => $chal_page->guid,
            'date_format' => $this->phpToJsDate(get_option('date_format'))
        ]);
        wp_enqueue_script($this->pluginName . "-spin", plugin_dir_url(__DIR__) . "includes/spin/spin.min.js", [
            'jquery'
        ], $this->version, false);
     ...
     * @return string
     */
    public static function phpToJsDate($php_format)
    {
     ...
     * @param string $tag
     */
    public static function displayChallengePage($attrs = [], $content = null, $tag = '')
    {
     ...
     */
    public static function displayChallengeListPage()
    {
     ...
     * @param string $tag
     */
    public static function displayActivityPage($attrs = [], $content = null, $tag = '')
    {
        $attrs = array_change_key_case((array) $attrs, CASE_LOWER);
        if (is_array($attrs) && count($attrs) && isset($attrs['chal'])) {
            self::$chal = $attrs['chal'];
     ...
     * @param object $part
     */
    public static function printActivity(&$act, &$part)
    {
        global $wpdb;
...
EOS;
    ...
    /**
     * Method to creating printer friendly page
     *
     * @global wpdb $wpdb
     *
     * @param object $act
     * @param object $part
     */
    public static function printerFriendly(&$act, &$part)
    {
        global $wpdb;
