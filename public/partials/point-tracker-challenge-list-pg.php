...
$chal_link = sanitize_text_field(
    filter_input(INPUT_GET, 'chal', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE)
);
$user = null;

if (!is_admin()) {
    $chal = PointTracker::init($chal_link, true);
    ...
    if (is_user_logged_in()) {
        ...
        if ($chal) {
            if (! $user || ! PointTracker::isUserInChallenge($chal->id, $user->user_id)) {
                // allow user to join
                PointTracker::joinChallenge($chal);
            } elseif (! PointTracker::isParticipantApproved($chal->id, $user->user_id)) {
...
EOL;
        ...
        wp_die("Viewing your current challenges requires you to <a href='".wp_login_url().
            "'>login</a>", "ACCOUNT_REQUIRED", [
            'response' => 301
        ]);
    }
...
<table id='registered-challenges'>
    ...
    foreach ($current_challenges as $chal) {
        ...
        $link = (
            $chal->approved ?
            "<a href='{$chal_page->guid}?chal={$chal->short_link}'>{$chal->short_link}</a>" :
            $chal->short_link
        );
...
<div id="dialog-form" title="Add new leader">
    ...
    <form>
        <fieldset>
            <input type="text" id="member-id" placeholder="Member ID..." inputmode='numeric' pattern='[0-9]*'
                class="text ui-widget-content ui-corner-all" />
            <input type="text" id="name" placeholder="Name..."
                value='<?php print $user->display_name; ?>'
                class="text ui-widget-content ui-corner-all" />
            <input type='email' id='email' placeholder='Email...'
                value='<?php print $user->user_email; ?>'
                class='text ui-widget-content ui-corner-all' />
