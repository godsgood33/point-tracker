...
ORDER BY al.log_date,al.log_time";
        ...
        if (is_array($log_res) && count($log_res)) {
            foreach ($log_res as $log) {
                ...
                $_data[] = [
                    ...
                    'action' => "<i class='far fa-trash-alt' ".
                        "title='Delete this activity so you can reinput with the correct info' ".
                        "data-act-id='{$log->activity_id}' data-log-date='{$dt->format("Y-m-d")}' ".
                        "data-user-id='{$log->user_id}'></i>"
                ];
            }
        }
     ...
     */
    public static function participantSaveEntry()
    {
        global $wpdb;
...
WHERE user_id = %d", $user_id);
     ...
     * @return string
     */
    public static function getMyActivityTable()
    {
        global $wpdb;
        $_data = [];
        $tp = 0;
        $member_id = filter_input(INPUT_POST, 'member-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
        $email = strtolower(sanitize_email(
            filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL, FILTER_NULL_ON_FAILURE)
        ));
        $chal_id = filter_input(INPUT_POST, 'chal-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
    ...
    email = %s", $chal_id, $member_id, $email);
        ...
        if (! $uid) {
            ...
            $tp = $wpdb->get_var($wpdb->prepare(
                "SELECT SUM(total_points) FROM {$wpdb->prefix}leader_board WHERE user_id = %d",
                $uid
            ));
...
ORDER BY
    al.log_date,al.log_time", $uid, $chal_id);
            ...
            foreach ($my_act as $act) {
                ...
                $_data[] = [
                    ...
                    'action' => "<i class='far fa-trash-alt' ".
                        "title='Delete this activity so you can input the correct info' ".
                        "data-act-id='{$act->id}' data-log-date='{$act->log_date}' data-user-id='{$act->user_id}'></i>"
                ];
            }
     ...
     */
    public static function deleteParticipantActivity()
    {
        global $wpdb;
        ...
        $act_id = filter_input(INPUT_POST, 'act-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
        $log_date = sanitize_text_field(filter_input(INPUT_POST, 'log-date', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE));
        ...
        print json_encode($res ? [
            ...
            'error' => "There was an error removing that activity, please contact the site admin ".
                get_option('admin-email', null)
        ]);
