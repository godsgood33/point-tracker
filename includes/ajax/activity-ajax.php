...
/**
 * Class to store ajax for activity data
 *
...
class ActivityAjax
{
    ...
    public static function getActivityTable()
    {
        ...
        $query = $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}pt_activities WHERE challenge_id = %d ORDER BY `order`",
            $chal_id
        );
        ...
        foreach ($res as $row) {
            if ($row->hidden) {
                $name = "<i title='Hidden activity, not used for point counts'>" .
                html_entity_decode($row->name, ENT_QUOTES | ENT_HTML5) . "*</i>";
            } else {
            ...
            $_data[] = [
                ...
                'extras' => (
                    $row->label ? html_entity_decode($row->label, ENT_QUOTES | ENT_HTML5) : "{$row->min}/{$row->max}"
                ),
                'action' => "<i class='fas fa-edit' data-id='{$row->id}'></i>&nbsp;&nbsp;".
                    "<i class='far fa-trash-alt' data-id='{$row->id}'></i>",
        ...
        $query = $wpdb->prepare(
            "SELECT COUNT(1) FROM {$wpdb->prefix}pt_activities 
        WHERE challenge_id = %d AND (`group` IS NOT NULL AND `group` != '')",
            $chal_id
        );
    ...
    public static function saveActivity()
    {
        ...
        $name = preg_replace("/[^a-z0-9]/", "", strtolower(
            sanitize_text_field(filter_input(INPUT_POST, 'name', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE))
        ));
        $act_id = filter_input(INPUT_POST, 'act-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
        ...
        $ques = sanitize_text_field(filter_input(INPUT_POST, 'question', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE));
        $desc = sanitize_text_field(filter_input(INPUT_POST, 'desc', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE));
        ...
        $group = sanitize_text_field(filter_input(INPUT_POST, 'group', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE));
        ...
        if (! self::validateActivity($params)) {
            print json_encode([
                'error' => $params['error']
            ]);
            wp_die();
        ...
        $query = $wpdb->prepare(
            "SELECT COUNT(1) FROM {$wpdb->prefix}pt_activities WHERE name = %s AND challenge_id = %d AND id != %d",
            $name,
            $chal_id,
            $act_id
        );
        ...
        $query = $wpdb->prepare(
            "SELECT COUNT(1) FROM {$wpdb->prefix}pt_activities WHERE challenge_id = %d",
            $chal_id
        );
     ...
     */
    public static function validateActivity(&$act)
    {
        $ret = true;
        ...
        if (in_array($act['type'], [
            ...
            $label = sanitize_text_field(filter_input(INPUT_POST, 'label', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE));
            if (! $label) {
     ...
     * @return string
     */
    public static function groupActivity()
    {
        global $wpdb;

        $chal_id = filter_input(INPUT_POST, 'chal-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
        $term = "%" . $wpdb->esc_like(
            stripslashes(filter_input(INPUT_POST, 'keyword', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE))
        ) . "%";
...
WHERE challenge_id = %d AND `group` LIKE %s",
            $chal_id,
            $term
        ));
