 ...
 */
    public static function getChallenge()
    {
        ...
        $chal->act_count = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(1) FROM {$wpdb->prefix}pt_activities WHERE challenge_id=%d",
            $chal_id
        ));
        $chal->part_count = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(1) FROM {$wpdb->prefix}pt_participants WHERE challenge_id=%d",
            $chal_id
        ));

        if (is_numeric($chal->winner)) {
            $chal->winner = $wpdb->get_var($wpdb->prepare(
                "SELECT name FROM {$wpdb->prefix}pt_participants WHERE challenge_id = %d AND user_id = %d",
                $chal_id,
                $chal->winner
            ));
        }
    ...
    public static function saveChallenge()
    {
        ...
        $req_start_date = sanitize_text_field(
            filter_input(INPUT_POST, 'start-date', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE)
        );
        $req_end_date = sanitize_text_field(
            filter_input(INPUT_POST, 'end-date', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE)
        );
        ...
        $name = sanitize_text_field(
            filter_input(INPUT_POST, 'name', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE)
        );
        ...
        $params = [
            ...
            'desc' => sanitize_textarea_field(
                filter_input(INPUT_POST, 'desc', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE)
            )
        ];
        if ($chal_id) {
     ...
     */
    public static function deleteChallenge()
    {
        global $wpdb;
        ...
        if ($res) {
            $activities = $wpdb->get_results($wpdb->prepare(
                "SELECT id FROM `{$wpdb->prefix}pt_activities` WHERE challenge_id=%d",
                $chal_id
            ));
    ...
    public static function getWidgetData()
    {
        ...
        if ($type == 'challenge') {
            ...
            $wpdb->query("SET @challenge_id = $chal_id");
            $query = "SELECT participant_name, total_points 
                FROM {$wpdb->prefix}leader_board ORDER BY total_points DESC LIMIT 1";
            ...
            $query = $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}pt_participants WHERE challenge_id = %d ORDER BY `name`",
                $chal_id
            );
            $rows = $wpdb->get_results($query);
     ...
     * @global wpdb $wpdb
     */
    public static function removeWinner()
    {
        global $wpdb;
