...
/**
 * Class to store ajax for participants
 *
 * @author Ryan Prather <godsgood33@gmail.com>
 */
class ParticipantAjax
{
...
WHERE
    ...
    public static function addParticipant()
    {
        ...
        $name = sanitize_text_field(
            filter_input(INPUT_POST, 'user-name', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE)
        );
        $email = strtolower(sanitize_email(
            filter_input(INPUT_POST, 'user-email', FILTER_SANITIZE_EMAIL, FILTER_NULL_ON_FAILURE)
        ));
     ...
     */
    public static function joinChallenge()
    {
        global $wpdb;

        $chal_link = sanitize_text_field(filter_input(INPUT_POST, 'chal-link', FILTER_DEFAULT, FILTER_NULL_ON_FAILURE));
        $member_id = filter_input(INPUT_POST, 'member-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
        ...
        $email = strtolower(sanitize_email(
            filter_input(INPUT_POST, 'email', FILTER_VALIDATE_EMAIL, FILTER_NULL_ON_FAILURE)
        ));
     ...
     * @global wpdb $wpdb
     */
    public static function clearParticipants()
    {
        global $wpdb;
        $chal_id = filter_input(INPUT_POST, 'chal-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);

        $query = $wpdb->prepare(
            "DELETE l.* FROM {$wpdb->prefix}pt_log l 
            JOIN {$wpdb->prefix}pt_participants p ON p.user_id = l.user_id 
            WHERE p.challenge_id = %d",
            $chal_id
        );
     ...
     * @global wpdb $wpdb
     */
    public static function markWinner()
    {
        global $wpdb;
        $chal_id = filter_input(INPUT_POST, 'chal-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
        $user_id = filter_input(INPUT_POST, 'user-id', FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
        ...
        print json_encode([
            'success' => "Congratulations, {$user->name}, has been marked the winner of the {$chal->name}"
        ]);
        wp_die();
    }
}
